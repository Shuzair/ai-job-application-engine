...
- `cv.docx`, `cv.pdf`, `cover-letter.docx`, `cover-letter.pdf`
...
4. Converts to PDF via LibreOffice (if available)

To adjust formatting (fonts, colors, margins, section order, etc.), edit the style's format config directly — no AI involved.
...
| Command | Purpose |
|---|---|
| `/apply` | Reads input.md, creates folder, optionally researches company, delegates to agents for YAML data |
| `/rerun <agent> [folder]` | Re-runs a single agent (`cv`, `cl`, or `research`) on an existing folder |
| `/format [folder] [style]` | Renders YAML data to docx + pdf using style-resolved format config. Style arg required for generic-cv/ folders |
| `/revise <cv\|cl> [folder] "feedback"` | Makes targeted edits to YAML data based on user feedback without full regeneration |
